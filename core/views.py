...
def generate_user_notifications(user):
    ...
    # 0. System/admin notifications (stored in DB)
    db_notes = Notification.objects.filter(user=user).order_by('-created_at')
...
def user_dashboard(request):
    ...
    # Ensure EMIs exist for approved loans
    for loan in active_loans:
        generate_emis_for_loan(loan)
...
def loan_status(request):
    ...
    documents_by_type = {
        'Personal Loan': [
            ...
            'Income proof (salary slip or bank statement, last 3–6 months)',
            'Address proof (utility bill or ward recommendation)',
        ],
        'Home Loan': [
            'Citizenship certificate (original)',
            'Recent passport-size photos (2)',
            'Land ownership certificate (lalpurja) — original',
            'Land tax clearance (Malpot) — latest',
            'Building permit/blueprint (if applicable)',
            'Income proof (salary slip or bank statement, last 6 months)',
        ],
        'Education Loan': [
            'Citizenship certificate (borrower/guardian)',
            'Recent passport-size photos (2)',
            'Admission letter/offer letter (original)',
            'Fee structure from institution',
            'Academic certificates/mark sheets',
            'Income proof of guardian (salary slip or bank statement)',
        ],
        'Business Loan': [
            'Citizenship certificate (original)',
            'Recent passport-size photos (2)',
        ...
        'Agriculture Loan': [
            ...
            'Recent passport-size photos (2)',
            'Land ownership certificate (lalpurja) — original',
            'Ward recommendation for farming',
            'Agriculture plan / estimate',
            'Bank statements (last 6 months)',
        ],
    }
    for loan in loans:
        ...
        if loan.status == 'approved':
            generate_emis_for_loan(loan)
        loan_type = (loan.loan_type or '').strip()
        loan.required_documents = documents_by_type.get(loan_type) or [
            'Citizenship certificate (original)',
            'Recent passport-size photos (2)',
            'Income proof (salary slip or bank statement, last 3–6 months)',
            'Address proof (utility bill or ward recommendation)',
    ...
    context = {
        ...
        'verification_updates': (
            [{
                'message': (
                    (note.message.split('\\n')[0].splitlines()[0] if note.message else '')
                ),
                'created_at': note.created_at,
            }
            for note in Notification.objects.filter(user=request.user).order_by('-created_at')
            if note.message and 'VERIFIED' in note.message][:1]
        ),
    }
...
def admin_applicants(request):
    ...
    members = Member.objects.filter(loan__isnull=False).distinct().order_by('-joined_date')
...
def admin_applicant_detail(request, member_id):
    if not request.user.is_staff:
        return redirect('login')
    ...
    summary = {
        ...
        'total_borrowed': loans.filter(status__in=['approved', 'closed']).aggregate(
            total=Sum('amount')
        )['total'] or 0,
    }
    ...
    return render(request, 'core/admin_applicant_detail.html', context)
...
def admin_reports(request):
    ...
    context = {
        ...
        'pending_pct': percent(pending_loans, total_loans),
        'approved_pct': percent(approved_loans, total_loans),
        'closed_pct': percent(closed_loans, total_loans),
        'recent_loans': recent_loans,
    }
...
def admin_settings(request):
    ...
    return render(request, 'core/admin_settings.html', {'member': member})
...
def admin_loan_detail(request, loan_id):
    ...
    detail = LoanApplicationDetail.objects.filter(loan=loan).first()

    if request.method == 'POST':
        action = request.POST.get('action')

        if loan.status != 'pending':
            messages.info(request, f"Loan #{loan.id} has already been processed.")
            return redirect('admin_loan_detail', loan_id=loan.id)
        ...
        if action == 'verify':
            ...
            docs = documents_by_type.get(loan_type) or [
                'Citizenship certificate (original)',
                'Recent passport-size photos (2)',
                'Income proof (salary slip or bank statement, last 3–6 months)',
                'Address proof (utility bill or ward recommendation)',
            ...
            Notification.objects.create(
                ...
                message=(
                    ...
                    f"Please visit our office with all original documents.\\n"
                    f"Required documents for {loan_type or 'your loan'}:\\n{documents_text}"
                )
            )
    ...
    return render(
        request,
        'core/admin_loan_detail.html',
        {'loan': loan, 'detail': detail}
    )
...
def generate_emis_for_loan(loan):
    ...
    for i in range(N):
        due_date = loan.issued_date + timedelta(days=30 * (i + 1))
        EMI.objects.create(
            loan=loan,
            due_date=due_date,
            amount=emi_amount
...
from django.contrib import messages
from django.utils import timezone
from .models import EMI, Payment, Loan, Notification, LoanApplicationDetail
